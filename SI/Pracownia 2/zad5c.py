...
# modyfikacja 1: eliminacja pól, do których nie da się dotrzeć (unikamy krążenia w kółko)
def reachable_bfs():
    reachable = set()
    queue = deque(goals)
    while queue:
...
class State:
    ...
    # modyfikacja 2: suma zamiast maxa, nie skupiamy się na najgorszym przypadku tylko uśredniamy
    def get_h(self, h):
        return sum(h.get(commando, float('inf')) * self.alpha for commando in self.commandos)

def move(state, move, h, alpha, reachable):
    new_commandos = set()
    move_tuple = moves[move]
    for commando in state.commandos:
        new_pos = (commando[0] + move_tuple[0], commando[1] + move_tuple[1])
        if new_pos in reachable and new_pos not in walls:
...
def main():
    alpha = 1.1 # mniejsza alpha dla krótszych ścieżek
    with open("zad_input.txt", "r") as fin, open("zad_output.txt", "w") as fout:
        ...
        for line in fin:
            ...
            y += 1        
        h = h_bfs()
        reachable = reachable_bfs()
        commandos.intersection_update(reachable)
        ...
        while pq:
            ...
            for dir in ["U", "D", "L", "R"]:
                new_state = move(current, dir, h, alpha, reachable)
