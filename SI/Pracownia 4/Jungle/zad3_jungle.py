...
class Player(object):
    ...
    def simulate_random_game(self, game_state, max_moves=100):
        ...
        if game.winner is None:
            return 0
        return 1 if game.winner == self.my_player else -1

    def choose_best_move(self, N=20000, max_moves_per_sim=100):
        moves = self.game.moves(self.my_player)
        if not moves:
            return None
        scores = {move: 0 for move in moves}
        simulations_per_move = max(1, N // (len(moves) * max_moves_per_sim))
        for move in moves:
            for _ in range(simulations_per_move):
                game_copy = copy.deepcopy(self.game)
                game_copy.do_move(move)
                result = self.simulate_random_game(game_copy, max_moves_per_sim)
                scores[move] += result
        best_move = max(moves, key=lambda m: scores[m])
        return best_move
    ...
    def loop(self):
        while True:
            ...
            if cmd == 'HEDID':
                ...
                assert cmd == 'UGO'
                self.my_player = 0
            move = self.choose_best_move(N=20000)
            if move:
                self.game.do_move(move)
