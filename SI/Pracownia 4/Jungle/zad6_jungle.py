...
class Player(object):
    ...
    def loop(self):
        while True:
            ...
            if cmd == 'HEDID':
                ...
                assert cmd == 'UGO'
                self.my_player = 0
            move = self.select_best_move(self.my_player)
            if move:
                self.game.do_move(move)
    ...
    def select_best_move(self, player):
        ...
        for move in moves:
            ...
            x2, y2 = pos2
            score = 0
            # Ruch do legowiska przeciwnika (wysoka nagroda)
            if pos2 == Jungle.dens[1 - player]:
                score += 1000
            # Bonus: zbijanie przeciwnika
            target = self.game.board[y2][x2]
            if target:
                enemy_player, enemy_piece = target
                if enemy_player != player:
                    score += Jungle.PIECE_VALUES[enemy_piece] * 10
            # Kara: narażenie się na zbicie przez silniejszego wroga
            for dx, dy in Jungle.dirs:
                nx, ny = x2 + dx, y2 + dy
                if 0 <= nx < Jungle.MX and 0 <= ny < Jungle.MY:
                    neighbor = self.game.board[ny][nx]
                    if neighbor and neighbor[0] != player:
                        enemy_piece = neighbor[1]
                        my_piece = self.game.board[pos1[1]][pos1[0]][1]
                        if self.game.can_beat(enemy_piece, my_piece, (nx, ny), (x2, y2)):
                            score -= Jungle.PIECE_VALUES[my_piece] * 8  # kara za ryzyko
            # Zbliżanie się do legowiska przeciwnika
            dens_x, dens_y = Jungle.dens[1 - player]
            dist = abs(x2 - dens_x) + abs(y2 - dens_y)
            score += (20 - dist)
            # Delikatny bonus za ruch (żeby unikać pasywności)
            if pos1 != pos2:
                score += 1
            if score > best_score:
                best_score = score
