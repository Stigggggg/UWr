...
const vector<pair<int, int>> X_SQUARES = {{1,1}, {1,6}, {6,1}, {6,6}};
const vector<pair<int, int>> C_SQUARES = {{0,1}, {1,0}, {0,6}, {1,7}, {6,0}, {7,1}, {6,7}, {7,6}};
...
int evaluate(const Reversi& game, int player) {
    ...
    for (auto [x, y] : CORNERS) {
        if (game.board[y][x] == player)
            score += 50;
        else if (game.board[y][x] == 1 - player)
            score -= 50;
    }
    //X_SQUARES - pozycje po przekątnej od narożników
    for (auto [x, y] : X_SQUARES) {
        if (game.board[y][x] == player)
            score -= 40;
        else if (game.board[y][x] == 1 - player)
            score += 40;
    }
    //C_SQUARES - pozycje sąsiadujące z rogami krawędzią
    for (auto [x, y] : C_SQUARES) {
        if (game.board[y][x] == player)
            score -= 15;
        else if (game.board[y][x] == 1 - player)
            score += 15;
    }
    //stabilny dysk - taki, że nie da się go już przejąć
    //stabilnymi dyskami będą narożniki i linie od narożników, ciągłe i tego samego koloru
    auto count_stable_discs = [&](int player_id) -> int {
        int stable = 0;
        for (auto [cx, cy] : CORNERS) {
            if (game.board[cy][cx] != player_id)
                continue;
            for (auto [dx, dy] : DIRS) {
                int x = cx + dx, y = cy + dy;
                while (x >= 0 && x < M && y >= 0 && y < M && game.board[y][x] == player_id) {
                    stable++;
                    x += dx;
                    y += dy;
                }
    ...
    int stable_me = count_stable_discs(player);
    int stable_op = count_stable_discs(1 - player);
    score += (stable_me - stable_op) * 20;
...
public:
    ...
    void loop() {
        ...
        while (true) {
            ...
            if (!moves.empty()) {
                move = minimax(game, 4, numeric_limits<int>::min(), numeric_limits<int>::max(), true, my_player).second;
            }
            if (move) {
