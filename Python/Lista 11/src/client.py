...
url = "http://localhost:5000/ksiazki"

"""Metody ktore moze wykonac klient na url"""
def get_ksiazki():
    response = requests.get(url)
    ksiazki = response.json()
    for ksiazka in ksiazki:
        print(
            f"{ksiazka['id']}: {ksiazka['tytul']} - {ksiazka['autor']} ({ksiazka['rok']})")
