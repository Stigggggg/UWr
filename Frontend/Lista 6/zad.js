//Wyzwanie 1 - const się nie hoistuje, function już tak
...
//Wyzwanie 3 - listę zastępujemy setem
const ids = [];
const generateId = () => {
  let id = 0;
  do {
...
function testAddingBooks(testCases) {
    for (const { testCase, shouldFail } of testCases) {
        try {
          ...
          if (shouldFail) {
            console.log("Test failed (should throw):", testCase);
          } else {
          ...
          if (shouldFail) {
            ...
            console.log("Test failed (unexpected error):", testCase, "| Error:", e.message);
          }
        }
    }    
}
...
//Wyzwanie 7
function addBooksToLibrary(bookList) {
    for (let bookArgs of bookList) {
