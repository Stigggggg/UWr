...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{let {SYMBOL ANY} ... ANY} s)
     (parse-binding-let (betweenlastfirst (s-exp->list s)) (last (s-exp->list s)))]
    [(s-exp-match? `{let* {SYMBOL ANY} ... ANY} s)
     (parse-binding-let* (betweenlastfirst (s-exp->list s)) (last (s-exp->list s)))]
    [else (error 'parse "invalid input")]))

(define (betweenlastfirst list)
...
(define (parse-binding-let [bindings : (Listof S-Exp)] [body : S-Exp]) : Exp
...
(define (extend-let env bindings)
  (if (empty? bindings)
      env
      (extend-let (extend-env env (fst (first bindings))
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE bindings body)
     (eval body (extend-let env bindings))]
    [(let*E bindings body)
     (eval body (extend-let* env bindings))]))
