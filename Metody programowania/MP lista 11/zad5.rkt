...
;remove z wykladu, usuwa element x ze zbioru (listy)
...
(define (fv [e : Exp]) : (Setof Symbol)
  (type-case Exp e
    [(numE n) (emptyS)] ; liczba nie moze byc zmienna wolna
    [(opE o l r) (unionS (fv l) (fv r))] ; rozklad wyrazenia i rekurencyjne sprawdzenie lewego i prawego wyrazenia
    [(ifE b l r) (unionS (fv b) (unionS (fv l) (fv r)))] ; to samo konstrukcja if
    [(varE x) (singletonS x)] ; jezeli to jest zmienna to dodajemy do listy
    [(letE x e1 e2) (unionS (fv e1) (removeS x (fv e2)))] ; usuwamy wszytsko to co jest zwiazane wyrazeniem let
    [(lamE x e) (removeS x (fv e))] ; usuwamy wszystkie zwiazane argumenty lambdy
    [(appE e1 e2) (unionS (fv e1) (fv e2))]))

;appE reprezentuje aplikację funkcji do argumentu; w pierwszym argumencie mamy wyrazenie a w drugim to co 
;bedziemy podstawiac do wyrazenia. stad nalezy rekurencyjnie wywolac sie na obu wyrazeniach

(module+ test
  (test (fv (parse `{double 9}))
        (fromList (list 'double)))
  (test (fv (parse `{{lambda {x} {+ x 1}} 5}))
        (emptyS))
  (test (fv (parse `{{lambda {x} {+ x 1}} y}))
        (fromList '(y)))
  (test (fv (parse `{{lambda {x} {+ x 1}} x}))
        (fromList '(x))))

(define exp1 (parse `{let x 5 (+ y 3)}))
(define exp2 (parse `{lambda (x) (+ x (+ y z))}))
(define exp3 (parse `{+ x x}))
(fv exp1)
(fv exp2)
(fv exp3)
