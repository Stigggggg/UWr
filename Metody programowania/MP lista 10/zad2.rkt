...
(define (decompile-helper [c : Code] [s : (Listof Exp)]) : Exp
  (if (empty? c) ;jesli nie ma instrukcji
      (first s)  ;to bierzemy ze stosu
      (type-case Instr (first c) ;jesli sa instrukcje, sprawdzamy po kolei, jakie sa
        [(pushI n) ;instrukcja polozenia liczby na stosie
         (decompile-helper (rest c) (cons (numE n) s))] ;rekurencyjne wywolanie dla reszty kodu i stosu powiekszonego o n na gorze
        [(opI op) ;instrukcja operacji
         (decompile-helper (rest c) (cons (opE op (second s) (first s)) (rest (rest s))))]))) 
