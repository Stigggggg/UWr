...
(define-type Binding
  (bind [name : Symbol]
;; zamiast wartosci trzymamy funkcje zwracające wartość
        [val : (-> Value)])) 
...
(define mt-env empty)
(define (extend-env [env : Env] [x : Symbol] [v : (-> Value)]) : Env ; tu tez ->
  (cons (bind x v) env))
(define (lookup-env [n : Symbol] [env : Env]) : Value
  (type-case (Listof Binding) env
    ...
    [(cons b rst-env) (cond
                        [(eq? n (bind-name b))
                         ((bind-val b))] ;jedne nawiasy wiecej bo wywolujemy funkcje
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE x e1 e2)
     (eval e2 (extend-env env x (lambda () (eval e1 env))))] ;musi byc zachowany typ binding, czyli 0-arg funkcja, ktora zwraca wartosc
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) (lambda () (eval e2 env)))]))

(define (apply [v1 : Value] [v2 : (-> Value)]) : Value ; w apply dajemy -> zamiast samo value
  (type-case Value v1
    [(funV x b env)
